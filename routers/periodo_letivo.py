...
def criar_periodo_letivo(periodo: PeriodoLetivoCreate, session: Session = Depends(get_session)):
...
def atualizar_periodo_letivo(periodo_id: int, dados: PeriodoLetivoUpdate, session: Session = Depends(get_session)):
    periodo = session.get(PeriodoLetivo, periodo_id)
    if not periodo:
        raise HTTPException(status_code=404, detail="Período letivo não encontrado")
...
def deletar_periodo_letivo(periodo_id: int, session: Session = Depends(get_session)):
    periodo = session.get(PeriodoLetivo, periodo_id)
    if not periodo:
        raise HTTPException(status_code=404, detail="Período letivo não encontrado")
