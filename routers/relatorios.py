...
from auth import permitir_todos

router = APIRouter(prefix="/relatorios", tags=["Relatórios"])


@router.get("/alunos-por-curso")
def quantidade_alunos_por_curso(
    ...
    return [
        {"curso": nome, "quantidade_alunos": qtd}
...
def quantidade_alunos_por_turma(
    ...
    return [
        {"turma": codigo, "quantidade_alunos": qtd}
        for codigo, qtd in resultado
    ]


@router.get("/turmas-por-professor")
def quantidade_turmas_por_professor(
    session: Session = Depends(get_session),
    usuario: Usuario = Depends(permitir_todos),
):
    resultado = session.exec(
    ...
    return [
        {"professor": nome, "quantidade_turmas": qtd}
        for nome, qtd in resultado
    ]


@router.get("/disciplinas-por-curso")
def quantidade_disciplinas_por_curso(
    session: Session = Depends(get_session),
    usuario: Usuario = Depends(permitir_todos),
):
    resultado = session.exec(
    ...
    return [
        {"curso": nome, "quantidade_disciplinas": qtd}
        for nome, qtd in resultado
    ]
...
def media_da_turma(
    ...
    media = session.exec(
        select(func.avg(Desempenho.mediaFinal))
        .where(Desempenho.id_turma == turma_id)
    ).first()

    return {
        "turma": turma.codigo,
        "media_geral": float(media) if media is not None else None,
    }


@router.get("/aprovados-reprovados/{turma_id}")
def aprovados_reprovados_por_turma(
    turma_id: int,
    session: Session = Depends(get_session),
    usuario: Usuario = Depends(permitir_todos),
):
    ...
    return [
        {"situacao": situacao, "quantidade": qtd}
        for situacao, qtd in resultado
    ]


@router.get("/professor-mais-turmas")
def professor_com_mais_turmas(
    session: Session = Depends(get_session),
    usuario: Usuario = Depends(permitir_todos),
):
    resultado = session.exec(
    ...
    return {
        "professor": nome,
        "quantidade_turmas": qtd,
    }


@router.get("/curso-mais-alunos")
def curso_com_mais_alunos(
    session: Session = Depends(get_session),
    usuario: Usuario = Depends(permitir_todos),
):
    resultado = session.exec(
    ...
    if not resultado:
        raise HTTPException(
            status_code=404,
            detail="Nenhum aluno matriculado encontrado",
        )
...
def historico_academico_do_aluno(
    aluno_id: int,
    session: Session = Depends(get_session),
    usuario: Usuario = Depends(permitir_todos),
):
    ...
    matriculas = session.exec(
        select(RegistroMatricula).where(
            RegistroMatricula.id_aluno == aluno_id
        )
    ).all()
    ...
    return {
        ...
        "matriculas": [
            RegistroMatriculaPublic.model_validate(m)
            for m in matriculas
        ],
        "turmas": [
            TurmaPublic.model_validate(t)
            for t in turmas
        ],
        "desempenhos": [
            DesempenhoPublic.model_validate(d)
