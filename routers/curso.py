...
def criar_curso(curso: CursoCreate, session: Session = Depends(get_session)):
...
def atualizar_curso(curso_id: int, dados: CursoUpdate, session: Session = Depends(get_session)):
    curso = session.get(Curso, curso_id)
    if not curso:
        raise HTTPException(status_code=404, detail="Curso não encontrado")
...
def deletar_curso(curso_id: int, session: Session = Depends(get_session)):
    curso = session.get(Curso, curso_id)
    if not curso:
        raise HTTPException(status_code=404, detail="Curso não encontrado")
