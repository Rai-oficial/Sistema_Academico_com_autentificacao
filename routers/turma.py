...
from models import Turma, Aluno, Aluno_Turma
from schemas import (
...
def criar_turma(turma: TurmaCreate, session: Session = Depends(get_session)):
...
def matricular_aluno_na_turma(turma_id: int, aluno_id: int, session: Session = Depends(get_session)):
...
def atualizar_turma(turma_id: int, dados: TurmaUpdate, session: Session = Depends(get_session)):
    turma = session.get(Turma, turma_id)
    if not turma:
        raise HTTPException(status_code=404, detail="Turma não encontrada")
...
def deletar_turma(turma_id: int, session: Session = Depends(get_session)):
    turma = session.get(Turma, turma_id)
    if not turma:
        raise HTTPException(status_code=404, detail="Turma não encontrada")
